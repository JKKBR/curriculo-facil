...
/* ====================== MENU, SOBRE O PROJETO, CONTADOR E FEEDBACK ====================== */
(function () {
  const COUNT_API_NAMESPACE = 'curriculo-for-ats';
  const COUNT_API_KEY = 'beneficiados';
  const BENEFIT_LOCAL_KEY = 'curriculo-beneficiado-v1';
  const GITHUB_FEEDBACK_URL = 'https://github.com/SEU_USUARIO/SEU_REPOSITORIO/issues/new';
  ...
  function initStructure() {
    ...
    if (!layout || document.querySelector('.site-header')) return;

    layout.id = 'criador-curriculo';

    const header = createEl('header', 'site-header');
    header.innerHTML = `
      <div class="site-header-inner">
        <div class="site-brand">Criador de Currículo</div>
        <nav class="site-nav" aria-label="Menu principal">
          <a href="#criador-curriculo">Criador de Currículo</a>
    ...
    const intro = createEl('section', 'site-intro');
    intro.innerHTML = `
      <h1>Criador de Currículo</h1>
      <p>Monte seu currículo, acompanhe a pré-visualização em tempo real e exporte o resultado com mais facilidade.</p>
    `;
    document.body.insertBefore(intro, layout);

    const about = createEl('section', '');
    about.id = 'sobre-projeto';
    about.innerHTML = `
      <h2>Sobre o Projeto</h2>
      <p>Este projeto foi pensado para facilitar a criação de currículos profissionais, com foco em clareza, adaptação às vagas e uso direto no navegador.</p>
      <div class="sobre-grid">
        <article class="sobre-card"><h3>Processo guiado</h3><p>Os campos são organizados por seções para deixar o preenchimento mais simples.</p></article>
        <article class="sobre-card"><h3>Prévia em tempo real</h3><p>O currículo pode ser visualizado enquanto a pessoa preenche as informações.</p></article>
        <article class="sobre-card"><h3>Exportação prática</h3><p>O conteúdo pode ser baixado em PDF e TXT.</p></article>
        <article class="sobre-card"><h3>Foco em ATS</h3><p>O assistente de termos ajuda a alinhar o currículo com descrições de vagas.</p></article>
        <article class="sobre-card"><h3>Privacidade</h3><p>A proposta é funcionar localmente no navegador, sem exigir cadastro para usar.</p></article>
        <article class="sobre-card"><h3>Acessibilidade</h3><p>A ideia é democratizar o acesso a um currículo profissional e fácil de personalizar.</p></article>
      </div>
    `;
    layout.insertAdjacentElement('afterend', about);

    const footer = createEl('footer', 'site-footer');
    footer.innerHTML = `
      <div class="counter-strip" id="contador-beneficiados-strip">
        <div>
          <div><strong id="benefitCounterValue">...</strong> pessoas já criaram CVs na plataforma</div>
          <small>O número é atualizado quando alguém conclui a criação do currículo pela primeira vez neste navegador.</small>
        </div>
        <div>
          <a href="#criador-curriculo" style="color:#fff; font-weight:700; text-decoration:none; border:1px solid rgba(255,255,255,0.2); padding:10px 14px; border-radius:999px; display:inline-block;">Criar meu currículo</a>
        </div>
      </div>
    `;
    about.insertAdjacentElement('afterend', footer);

    const widget = createEl('section', 'feedback-widget');
    widget.innerHTML = `
      <button type="button" class="feedback-toggle" id="feedbackToggle">
        <span>💬 Enviar feedback</span>
        <span id="feedbackToggleIcon">+</span>
      </button>
      <div class="feedback-panel" id="feedbackPanel">
        <h3>Fale sobre a plataforma</h3>
        <p>Envie ideias de melhoria, reporte bugs, erros ou conte o que funcionou bem.</p>
        <input type="text" id="feedbackNome" placeholder="Seu nome (opcional)">
        <input type="email" id="feedbackEmail" placeholder="Seu e-mail (opcional)">
        ...
        <textarea id="feedbackMensagem" rows="5" placeholder="Descreva sua mensagem"></textarea>
        <div class="feedback-actions">
          <button type="button" id="feedbackEnviarGithub">Abrir no GitHub</button>
          <button type="button" class="btn-secundario" id="feedbackCopiarMensagem">Copiar mensagem</button>
        </div>
        <div class="feedback-status" id="feedbackStatus"></div>
        <div class="helper-note">Troque a URL do repositório no script.js para abrir issues já preenchidas.</div>
      </div>
    `;
    document.body.appendChild(widget);

    header.querySelectorAll('a').forEach(link => link.addEventListener('click', smoothScrollHandler));
    const footerLink = footer.querySelector('a');
    if (footerLink) footerLink.addEventListener('click', smoothScrollHandler);
  }

  async function atualizarContadorVisual() {
    const valueEl = document.getElementById('benefitCounterValue');
    if (!valueEl) return;
    try {
      const response = await fetch(`https://api.countapi.xyz/get/${COUNT_API_NAMESPACE}/${COUNT_API_KEY}`);
      if (!response.ok) throw new Error('contador');
      const data = await response.json();
      valueEl.textContent = Number(data.value || 0).toLocaleString('pt-BR');
    } catch (error) {
      const local = Number(localStorage.getItem('benefitCounterFallback') || '0');
      valueEl.textContent = local.toLocaleString('pt-BR');
    }
  }

  async function registrarBeneficioSeNecessario() {
    const today = new Date().toISOString().slice(0, 10);
    const benefitKey = `${BENEFIT_LOCAL_KEY}:${today}`;
    if (localStorage.getItem(benefitKey) === '1') {
    ...
    try {
      ...
      if (!response.ok) throw new Error('contador');
      const data = await response.json();
      localStorage.setItem(benefitKey, '1');
      localStorage.setItem('benefitCounterFallback', String(Number(data.value || 0)));
      const valueEl = document.getElementById('benefitCounterValue');
      if (valueEl) valueEl.textContent = Number(data.value || 0).toLocaleString('pt-BR');
    } catch (error) {
      const local = Number(localStorage.getItem('benefitCounterFallback') || '0') + 1;
      localStorage.setItem('benefitCounterFallback', String(local));
      localStorage.setItem(benefitKey, '1');
      const valueEl = document.getElementById('benefitCounterValue');
      if (valueEl) valueEl.textContent = local.toLocaleString('pt-BR');
    }
  }

  function montarMensagemFeedback() {
    const nome = document.getElementById('feedbackNome')?.value.trim() || 'Visitante';
    const email = document.getElementById('feedbackEmail')?.value.trim() || 'não informado';
    const tipo = document.getElementById('feedbackTipo')?.value || 'Feedback';
    const mensagem = document.getElementById('feedbackMensagem')?.value.trim() || '';
    return {
      titulo: `${tipo}: ${mensagem.slice(0, 60) || 'Nova mensagem do site'}`,
      corpo: `### Tipo\n${tipo}\n\n### Nome\n${nome}\n\n### E-mail\n${email}\n\n### Mensagem\n${mensagem}`
    };
  }

  function atualizarStatusFeedback(texto, erro) {
    const status = document.getElementById('feedbackStatus');
    if (!status) return;
    status.textContent = texto;
    status.style.color = erro ? '#b91c1c' : '#0f766e';
  }
  ...
  function initFeedback() {
    ...
    const openGithubBtn = document.getElementById('feedbackEnviarGithub');
    const copiarBtn = document.getElementById('feedbackCopiarMensagem');
    ...
    if (openGithubBtn) {
      openGithubBtn.addEventListener('click', function () {
        const mensagem = document.getElementById('feedbackMensagem')?.value.trim();
        if (!mensagem) {
          atualizarStatusFeedback('Escreva uma mensagem antes de enviar.', true);
          return;
        }
        const payload = montarMensagemFeedback();
        const separador = GITHUB_FEEDBACK_URL.includes('?') ? '&' : '?';
        const url = `${GITHUB_FEEDBACK_URL}${separador}title=${encodeURIComponent(payload.titulo)}&body=${encodeURIComponent(payload.corpo)}`;
        window.open(url, '_blank', 'noopener');
        atualizarStatusFeedback('Nova aba aberta para você enviar a issue no GitHub.', false);
      });
    }
    ...
    if (copiarBtn) {
      copiarBtn.addEventListener('click', async function () {
        const mensagem = document.getElementById('feedbackMensagem')?.value.trim();
        if (!mensagem) {
          atualizarStatusFeedback('Escreva uma mensagem antes de copiar.', true);
          return;
        }
        const payload = montarMensagemFeedback();
        try {
          await navigator.clipboard.writeText(payload.corpo);
          atualizarStatusFeedback('Mensagem copiada com sucesso.', false);
        } catch (error) {
          atualizarStatusFeedback('Não foi possível copiar automaticamente.', true);
        }
      });
    }
  }

  function interceptarAcoesDeBeneficio() {
    if (window.__benefitHooksRegistered) return;
    window.__benefitHooksRegistered = true;
    if (typeof window.gerarPDF === 'function') {
      const originalGerarPDF = window.gerarPDF;
      window.gerarPDF = function () {
        const result = originalGerarPDF.apply(this, arguments);
        registrarBeneficioSeNecessario();
        return result;
      };
    }
    if (typeof window.salvarComoTXT === 'function') {
      const originalSalvarComoTXT = window.salvarComoTXT;
      window.salvarComoTXT = function () {
        const result = originalSalvarComoTXT.apply(this, arguments);
        registrarBeneficioSeNecessario();
        return result;
      };
    }
  ...
  function corrigirDuplicidadeSugestoes() {
    const areas = document.querySelectorAll('#sugestoesPalavras');
    if (areas.length > 1) {
  ...
  window.addEventListener('DOMContentLoaded', function () {
    initStructure();
    corrigirDuplicidadeSugestoes();
    atualizarContadorVisual();
    initFeedback();
    interceptarAcoesDeBeneficio();
  });
})();
